...
그러면 고객의 불만이 발생할 수 있기 때문에 재고량을 관리가 필요합니다. 또 이렇게 관리한 데이터를 바탕으로 미래의 재고량 수급에 참고 할 수 있을 것입니다.
...
int main(void){
    ...
    // 주문할 개수 입력
    int order = get_int("주문하실 메론 개수를 입력해주세요: ");

    // 메론 가격
    int price = 10000;

    // 부가세포함 매출액 계산
    float sell = (price * order) * 1.1;

    // 기존 재고에 맞춰 재고가 있으면 계산되고 재고가 없으면 메세지 출력
    if(quantity > order){
